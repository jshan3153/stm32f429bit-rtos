...
uint32_t uartTerminal_send(uint8_t data);
...
//uint32_t uartTerminal_init(UART_HandleTypeDef *huart)
//{
//  sp_huart = huart;
...
uint32_t uartTerminal_recvTry(char *data)
{
...
void doDebugMonitor()
{
  ...
  if (uartTerminal_recvTry(&s_storedCommand[s_storedCommandIndex]) == 1) {
    /* echo back */
